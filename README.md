...
저장소 루트에서 통합 실행기를 사용하면 AI, Backend, Frontend를 함께 준비하고 실행합니다.
...
통합 실행기는 다음을 자동으로 처리합니다.

- `.env`가 없으면 `.env.example`에서 생성
- Node.js, Python, ffmpeg, ffprobe 확인
- 이동 후 깨진 `ai/.venv` 감지 및 재생성
- AI 및 Node 의존성 설치
- `STT_PROVIDER_OPTIONS`에 지정된 STT provider import 확인 및 설치
- `8000`, `3000`, `5173` 포트 충돌 확인
- AI를 반드시 `ai/.venv` Python으로 실행
- 세 서비스 중 하나가 종료되거나 Ctrl+C가 입력되면 나머지도 함께 종료

서버를 실행하지 않고 환경만 점검하려면:
...
Windows에서 남아 있는 서비스 프로세스를 정리하려면:
